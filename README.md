...
본 프로젝트의 메인 아이디어는 사용자의 감정에 기반한 컨텐츠 추천 시스템입니다. 사용자의 감정을 입력받아 감정을 분석하고, 분석된 감정을 세 가지로 분류해 알려드립니다. 기본적으로 사용자의 감정과 동일하게 분류된 컨텐츠를 추천해드리지만, 분류된 감정이 사용자의 감정과 맞지 않거나 사용자가 다른 감정을 원하는 경우 감정 없음을 포함하여 44개의 감정을 선택할 수 있습니다. 컨텐츠는 사용자가 선택한 최종 3가지의 감정에 기반해 최대 네 종류의 컨텐츠(노래, 책, 영화, 공연)를 추천해드립니다. 
